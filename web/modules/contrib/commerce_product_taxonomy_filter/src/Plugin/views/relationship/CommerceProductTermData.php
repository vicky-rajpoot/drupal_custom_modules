...
namespace Drupal\commerce_product_taxonomy_filter\Plugin\views\relationship;
...
class CommerceProductTermData extends RelationshipPluginBase {
  ...
  public function query() {
    ...
    if (!array_filter($this->options['vids'])) {
      $commerce_product_taxonomy_index = $this->query->addTable('commerce_product_taxonomy_index', $this->relationship);
      $def['left_table'] = $commerce_product_taxonomy_index;
      $def['left_field'] = 'tid';
      $def['field'] = 'tid';
      ...
      $query->addJoin($def['type'], 'commerce_product_taxonomy_index', 'tn', 'tn.tid = td.tid');
